...
function genesis_sample_customizer_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'genesis_sample_link_color',
			[
				'description' => __( 'Change the color of post info links and button blocks, the hover color of linked titles and menu items, and more.', 'immortals' ),
				'label'       => __( 'Link Color', 'immortals' ),
				'section'     => 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'genesis_sample_accent_color',
			[
				'description' => __( 'Change the default hover color for button links, menu buttons, and submit buttons. The button block uses the Link Color.', 'immortals' ),
				'label'       => __( 'Accent Color', 'immortals' ),
				'section'     => 'colors',
	...
	$wp_customize->add_control(
		'genesis_sample_logo_width',
		[
			'label'       => __( 'Logo Width', 'immortals' ),
			'description' => __( 'The maximum width of the logo in pixels.', 'immortals' ),
			'priority'    => 9,
...
function genesis_sample_validate_logo_width( $validity, $width ) {
	...
	if ( empty( $width ) || ! is_numeric( $width ) ) {
		$validity->add( 'required', __( 'You must supply a valid number.', 'immortals' ) );
	} elseif ( $width < 100 ) {
		$validity->add( 'logo_too_small', __( 'The logo width cannot be less than 100.', 'immortals' ) );
	}
